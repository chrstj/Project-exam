      ...
      const fetchPosts = async () => {
        try {
          const response = await fetch(
            "https://nf-api.onrender.com/api/v1/social/posts?_comments=true&_author=true&_reactions=true",
            {
              headers: {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (updatedPostsResponse.ok) {
            ...
            setError(
              "Error fetching updated posts: " + updatedPostsResponse.statusText
            );
          }
        } else {
  ...
  return (
    <div className="container-sm text-center">
      ...
      <div className="card-container justify-content-center align-items-center">
        <button className="btn btn-primary mb-3" onClick={toggleCreatePostForm}>
          Create New Post
        </button>
